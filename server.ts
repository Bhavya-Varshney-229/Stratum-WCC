...
// Helper for calling Gemini JSON schema
async function generateJson<T>(prompt: string, schema: any, systemInstruction?: string): Promise<T> {
  ...
  const response = await aiClient.models.generateContent({
    ...
    contents: prompt,
    config: {
      systemInstruction: systemInstruction || 'You are an elite venture strategist and brand intelligence expert. Return only valid JSON conforming strictly to the requested schema.',
